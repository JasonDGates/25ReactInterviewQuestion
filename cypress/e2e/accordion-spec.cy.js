describe("Accordion Component", () => {
  it("should toggle visibility when clicking on a card", () => {
    ...
    cy.get(".question").eq(0).click(); // Click the first card
    cy.get(".answer").eq(0).should("be.visible"); // Assert that the answer is visible

    cy.get(".question").eq(0).click(); // Click the first card again
    cy.get(".card").eq(1).should("not.contain", ".answer"); // Assert that the answer is not visible
  });
